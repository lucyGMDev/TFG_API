...
public class CommentController {
  public static Response postComment(final String token, final CommentBody comment) {
    ...
    if(comment.getResponseCommentId()!= null && !database.commentExistsInProject(comment.getResponseCommentId(), comment.getProjectId()))
    {
    ...
    if(database.addCommentToProject(comment.getProjectId(), commentId,comment.getCommentText(),comment.getWritterEmail(),comment.getResponseCommentId()) == -1)
    {
